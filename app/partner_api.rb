module FilesMockServer
  class PartnerAPI < Grape::API
    format :json

    params do
      optional :cursor, type: String
      optional :per_page, type: Integer
      optional :sort_by, type: Hash
      optional :filter, type: Hash
    end
    get "/api/rest/v1/partners" do
      status 200
      [ { "allow_bypassing_2fa_policies" => false, "allowed_ips" => "10.0.0.0/8\n127.0.0.1", "allow_credential_changes" => false, "allow_providing_gpg_keys" => false, "allow_user_creation" => false, "cc_emails_to_responsible_party" => false, "connections" => [ { "id" => 1, "role" => "guest", "site_id" => 2, "site_name" => "Acme Site", "mount_path" => "_/Sites/2" } ], "id" => 1, "ai_assistant_personality_id" => 1, "workspace_id" => 1, "name" => "Acme Corp", "notes" => "This is a note about the partner.", "partner_admin_ids" => [ 1, 2, 3 ], "partner_channel_template_id" => 1, "partnership_role" => "host", "responsible_group_id" => 1, "responsible_user_id" => 1, "root_folder" => "/AcmeCorp", "show_partner_channel_home_page" => false, "tags" => "example", "user_ids" => [ 1, 2, 3 ] } ]
    end

    params do
      optional :ai_assistant_personality_id, type: Integer
      optional :allowed_ips, type: String
      optional :allow_bypassing_2fa_policies, type: Boolean
      optional :allow_credential_changes, type: Boolean
      optional :allow_providing_gpg_keys, type: Boolean
      optional :allow_user_creation, type: Boolean
      optional :cc_emails_to_responsible_party, type: Boolean
      optional :notes, type: String
      optional :partner_channel_template_id, type: Integer
      optional :responsible_group_id, type: Integer
      optional :responsible_user_id, type: Integer
      optional :show_partner_channel_home_page, type: Boolean
      optional :tags, type: String
      requires :name, type: String
      requires :root_folder, type: String
      optional :workspace_id, type: Integer
    end
    post "/api/rest/v1/partners" do
      status 201
      { "allow_bypassing_2fa_policies" => false, "allowed_ips" => "10.0.0.0/8\n127.0.0.1", "allow_credential_changes" => false, "allow_providing_gpg_keys" => false, "allow_user_creation" => false, "cc_emails_to_responsible_party" => false, "connections" => [ { "id" => 1, "role" => "guest", "site_id" => 2, "site_name" => "Acme Site", "mount_path" => "_/Sites/2" } ], "id" => 1, "ai_assistant_personality_id" => 1, "workspace_id" => 1, "name" => "Acme Corp", "notes" => "This is a note about the partner.", "partner_admin_ids" => [ 1, 2, 3 ], "partner_channel_template_id" => 1, "partnership_role" => "host", "responsible_group_id" => 1, "responsible_user_id" => 1, "root_folder" => "/AcmeCorp", "show_partner_channel_home_page" => false, "tags" => "example", "user_ids" => [ 1, 2, 3 ] }
    end

    params do
      requires :id, type: Integer
    end
    get "/api/rest/v1/partners/:id" do
      status 200
      { "allow_bypassing_2fa_policies" => false, "allowed_ips" => "10.0.0.0/8\n127.0.0.1", "allow_credential_changes" => false, "allow_providing_gpg_keys" => false, "allow_user_creation" => false, "cc_emails_to_responsible_party" => false, "connections" => [ { "id" => 1, "role" => "guest", "site_id" => 2, "site_name" => "Acme Site", "mount_path" => "_/Sites/2" } ], "id" => 1, "ai_assistant_personality_id" => 1, "workspace_id" => 1, "name" => "Acme Corp", "notes" => "This is a note about the partner.", "partner_admin_ids" => [ 1, 2, 3 ], "partner_channel_template_id" => 1, "partnership_role" => "host", "responsible_group_id" => 1, "responsible_user_id" => 1, "root_folder" => "/AcmeCorp", "show_partner_channel_home_page" => false, "tags" => "example", "user_ids" => [ 1, 2, 3 ] }
    end

    params do
      requires :id, type: Integer
      optional :ai_assistant_personality_id, type: Integer
      optional :allowed_ips, type: String
      optional :allow_bypassing_2fa_policies, type: Boolean
      optional :allow_credential_changes, type: Boolean
      optional :allow_providing_gpg_keys, type: Boolean
      optional :allow_user_creation, type: Boolean
      optional :cc_emails_to_responsible_party, type: Boolean
      optional :notes, type: String
      optional :partner_channel_template_id, type: Integer
      optional :responsible_group_id, type: Integer
      optional :responsible_user_id, type: Integer
      optional :show_partner_channel_home_page, type: Boolean
      optional :tags, type: String
      optional :name, type: String
      optional :partnership_role, type: String
      optional :root_folder, type: String
    end
    patch "/api/rest/v1/partners/:id" do
      status 200
      { "allow_bypassing_2fa_policies" => false, "allowed_ips" => "10.0.0.0/8\n127.0.0.1", "allow_credential_changes" => false, "allow_providing_gpg_keys" => false, "allow_user_creation" => false, "cc_emails_to_responsible_party" => false, "connections" => [ { "id" => 1, "role" => "guest", "site_id" => 2, "site_name" => "Acme Site", "mount_path" => "_/Sites/2" } ], "id" => 1, "ai_assistant_personality_id" => 1, "workspace_id" => 1, "name" => "Acme Corp", "notes" => "This is a note about the partner.", "partner_admin_ids" => [ 1, 2, 3 ], "partner_channel_template_id" => 1, "partnership_role" => "host", "responsible_group_id" => 1, "responsible_user_id" => 1, "root_folder" => "/AcmeCorp", "show_partner_channel_home_page" => false, "tags" => "example", "user_ids" => [ 1, 2, 3 ] }
    end

    params do
      requires :id, type: Integer
    end
    delete "/api/rest/v1/partners/:id" do
      status 204
      body false
    end
  end
end
